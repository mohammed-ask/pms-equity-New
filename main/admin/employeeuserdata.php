...
include '../session.php';
/* @var $obj db */
$code = $obj->selectfieldwhere("users", "usercode", "id=" . $employeeid . " and status = 1");
$limit = $_GET['length'];
$start = $_GET['start'];
$i = 1;
...
$result = $obj->selectextrawhereupdate(
    "users ",
    "*",
    "status in (1) and type = 2 and id not in (26) and employeeref = '$code' $search $order limit $start, $limit"
);
...
while ($row = $obj->fetch_assoc($result)) {
    ...
    if (in_array(84, $permissions)) {
        $a .= "<button class='btn' onclick='redir(\"" . $row['id'] . "\",\"\",\"dashboard\",\"_blank\")'  aria-label='Go'>
             <span class='w-3 h-3'><i class='fa fa-duotone fa-share-from-square'></i></span>

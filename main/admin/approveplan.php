...
print_r($_GET);
$obj->saveactivity("Plan Request Request Approved/Disapprove", "", $_GET["hakuna"], $_GET["hakuna"], "User", "Plan Request Request Approved/Disapprove");
$id = $_GET['hakuna'];
...
if ($_GET['what'] === 'Approve' && $rowfund['status'] == 0) {
    ...
    //Add Subscriber
    $kk['added_on'] = date('Y-m-d H:i:s');
    $kk['added_by'] = $employeeid;
    $kk['updated_on'] = date('Y-m-d H:i:s');
    $kk['updated_by'] = $employeeid;
    $kk['status'] = 1;
    $kk['plandetailid'] = $rowfund['plandetailid'];
    $kk['paymentid'] = $rowfund['id'];
    $kk['userid'] = $rowfund['userid'];
    $kk['expireon'] = $expiredate;
    $obj->insertnew("subscribers", $kk);

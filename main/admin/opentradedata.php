...
while ($row = $obj->fetch_assoc($result)) {
    ...
    if (in_array(38, $permissions)) {
        $addaction = " <div style='display: inline-flex'><button style='margin-right: 10px;' class='btn' data-bs-toggle='modal' data-bs-target='#modal-report'  onclick='dynamicmodal(\"" . $row['id'] . "\", \"editstockprice\", \"\", \"Edit Stock Price\")'>
    ...
    if ($row['trademethod'] === 'Buy' && empty($row['stockid']) && in_array(39, $permissions)) {
        $addaction .=  "<button class='btn text-white btn-danger'  data-bs-toggle='modal' data-bs-target='#modal-report' onclick='dynamicmodal(\"" . $row['id'] . "\", \"closebrockertrade\", \"Buy\", \"Close Trade\")'>Sell</button>";
    } else if ($row['trademethod'] === 'Sell' && empty($row['stockid']) && in_array(39, $permissions)) {
        $addaction .=  "<button class='btn text-white btn-success' data-bs-toggle='modal' data-bs-target='#modal-report' onclick='dynamicmodal(\"" . $row['id'] . "\", \"closebrockertrade\", \"Sell\", \"Close Trade\")'>Buy</button></div>";
    }

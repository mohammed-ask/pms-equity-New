...
$return['recordsTotal'] = $obj->selectfieldwhere("stocktransaction $join", "count(stocktransaction.id)", "stocktransaction.status in (1,0) and stocktransaction.userid = $id and aitrade = 'Yes' ");
$return['recordsFiltered'] = $obj->selectfieldwhere("stocktransaction $join", "count(stocktransaction.id)", "stocktransaction.status in (1,0) and stocktransaction.userid = $id and aitrade = 'Yes'  $search ");
$return['draw'] = $_GET['draw'];
$result = $obj->selectextrawhereupdate(
    ...
    "stocktransaction.status in (1,0) and stocktransaction.userid = $id and aitrade = 'Yes' $search $order limit $start, $limit"
);
...
while ($row = $obj->fetch_assoc($result)) {
    ...
    $n[] = $row['tradestatus'] === 'Close' ? '<strong class="badge bg-label-danger">Closed<strong>' : '<strong class="text-success">Open<strong>';
    $data[] = $n;
    $i++;
}
$return['data'] = $data;
